...
struct State<S = StateApi> {
    ...
    token_metadata_urls: StateMap<ContractTokenId, String, S>,
    total_mints: u32,
...
#[init(contract = "techfiesta_minter_contract", event = "Event")]
...
fn contract_mint(
    ...
    // Increase the token_id_counter and total mints.
    host.state_mut().token_id_counter += 1;
    host.state_mut().total_mints += 1;
    ...
    logger.log(&Cis2Event::TokenMetadata::<_, ContractTokenAmount>(
        TokenMetadataEvent {
            ...
            metadata_url: MetadataUrl {
                url: String::from(&params.token_metadata_base_url),
                hash: None,
            },
        },
    ))?;
...
fn contract_permit(
    ...
    *entry += 1;
        ...
        == EntrypointName::new_unchecked("updateOperator")
    {
    ...
    } else if message.entry_point.as_entrypoint_name() == EntrypointName::new_unchecked("mint") {
        // Parse the mint parameters from the payload
        let MintParams {
            // token_id,
            owner,
            token_metadata_base_url,
        }: MintParams = from_bytes(&message.payload)?;
        ...
        match mint(mint_params, host, logger) {
            Ok(_) => {
                logger.log(&Cis2Event::Mint(MintEvent {
                    token_id,
                    amount: ContractTokenAmount::from(1),
