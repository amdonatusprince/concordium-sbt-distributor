...
pub async fn handle_signature_mint(
    key_update_operator: Arc<WalletAccount>,
    request: MintInputParams, // This should contain necessary mint parameters
    smart_contract_index: u64,
    state: Server,
) -> Result<impl warp::Reply, Rejection> {
    log::debug!("Create payload.");

    let transfers = MintData {
        owner: Address::Account(request.owner),
        token_metadata_base_url: request.token_metadata_base_url,
    };

    // let payload = MintParams(vec![transfer]);

    log::debug!("Create PermitMessage for mint.");
    ...
    let message: PermitMessage = PermitMessage {
        ...
        nonce: request.nonce,
        timestamp: request.timestamp,
        entry_point: OwnedEntrypointName::new_unchecked("mint".into()), // Specify the "mint" entry point
        // payload, // Use the mint parameters as payload
        payload: concordium_rust_sdk::smart_contracts::common::to_bytes(&transfers),
    };

    log::debug!("{}", request.signature);
    // Submit the mint transaction using the provided signature.
    submit_transaction(
        key_update_operator,
        state,
        message,
        request.signature, // Signature to verify the minting
        request.signer,    // Signer who authorized the minting
    )
    .await
}
